...
OUTROOT="$HOME/Fast-dLLM/results/cache_refresh_gate_$RUN_ID"
LOGROOT="$HOME/Fast-dLLM/logs/cache_refresh_gate_$RUN_ID"
...
for INTERVAL in 1 2; do
    ...
    echo
    echo "=================================================="
    echo "interval=$INTERVAL limit=$LIMIT"
    echo "started=$(date)"
    echo "=================================================="
    ...
    if [ "$STATUS" -ne 0 ]; then
        echo "interval=$INTERVAL failed status=$STATUS"
        FAILURES=$((FAILURES + 1))
    else
        echo "interval=$INTERVAL completed"
    fi
done
