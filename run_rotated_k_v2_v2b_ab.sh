...
OUTROOT="$HOME/Fast-dLLM/results/rotated_k_v2_v2b_ab_$RUN_ID"
LOGROOT="$HOME/Fast-dLLM/logs/rotated_k_v2_v2b_ab_$RUN_ID"
...
for REPEAT in $(seq 1 "$REPEATS"); do
    for MODE in v2 v2b; do
        if [ "$MODE" = "v2" ]; then
            TARGET="$ROOT/eval_llada_rotated_k_v2_reference.py"
        else
            TARGET="$ROOT/eval_llada_rotated_k_v2b_wrapper.py"
        fi
