...
import torch
from ffn_sparsity_ablation import probe_ffn_activation
...
class LLaDALlamaBlock(LLaDABlock):
    ...
    def __init__(self, layer_id: int, config: ModelConfig, cache: BufferCache):
        ...
        # Runtime-only FFN fusion cache (not in state_dict)
        self._ff_up_weight_cache = None
        self._ff_up_bias_cache = None
    ...
    def forward(
        ...
        if self._activation_checkpoint_fn is not None:
            ...
            x = self.ff_norm(x)
        if self._ff_up_weight_cache is None:
            self._ff_up_weight_cache = torch.cat(
                [
                    self.ff_proj.weight,
                    self.up_proj.weight,
                ],
            ...
            if self.ff_proj.bias is not None:
                self._ff_up_bias_cache = torch.cat(
                    [
                        self.ff_proj.bias,
                        self.up_proj.bias,
                    ],
        ...
        x, x_up = F.linear(
            x,
            self._ff_up_weight_cache,
            self._ff_up_bias_cache,
        ).chunk(2, dim=-1)  # FFN fusion
        if self._activation_checkpoint_fn is not None:
            x = self._activation_checkpoint_fn(self.act, x)  # type: ignore
        else:
            x = self.act(x)
